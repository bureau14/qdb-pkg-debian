...
QDB_ALL=$(ls qdb-all/*.deb)
QDB_API=$(ls qdb-api/*.deb)
...
RESULT=$(sudo lxc-attach --clear-env -n $CONTAINER_NAME -- qdbsh --cluster-public-key-file=/usr/share/qdb/cluster_public.key --user-credentials-file=/etc/qdb/qdbsh_private.key -c "blob_get hello") || echo "##teamcity[testFailed name='qdbsh.get' message='Failed to get blob']"
...
RESULT=$(sudo lxc-attach --clear-env -n $CONTAINER_NAME -- qdbsh --cluster-public-key-file=/usr/share/qdb/cluster_public.key --user-credentials-file=/etc/qdb/qdbsh_private.key -c "blob_get hello") || echo "##teamcity[testFailed name='qdbsh.get.after-reboot' message='Failed to get blob']"
[ "$RESULT" = "world" ] || echo "##teamcity[testFailed name='qdbsh.get' message='Invalid output from blob_get']"
...
sudo lxc-attach --clear-env -n $CONTAINER_NAME -- dpkg -r qdb-server || echo "##teamcity[testFailed name='server.uninstall' message='Failed to uninstall server']"
...
sudo lxc-attach --clear-env -n $CONTAINER_NAME -- dpkg -r qdb-web-bridge || echo "##teamcity[testFailed name='web-bridge.uninstall' message='Failed to uninstall web-bridge']"
