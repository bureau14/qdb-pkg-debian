...
function run_test_on_container {
    ...
    sudo lxc-attach --clear-env --set-var DEBIAN_FRONTEND=noninteractive -n $CONTAINER_NAME -- apt-get install -y wget
    ...
    sudo lxc-attach --clear-env -n $CONTAINER_NAME -- qdbsh -c "blob_put hello world" || echo "##teamcity[testFailed name='qdbsh.put' message='Failed to put blob']"
    echo "##teamcity[testFinished name='qdbsh.put']"

    echo "##teamcity[testStarted name='qdbsh.get' captureStandardOutput='true']"
    RESULT=$(sudo lxc-attach --clear-env -n $CONTAINER_NAME -- qdbsh -c "blob_get hello") || echo "##teamcity[testFailed name='qdbsh.get' message='Failed to get blob']"
    ...
    RESULT=$(sudo lxc-attach --clear-env -n $CONTAINER_NAME -- qdbsh -c "blob_get hello") || echo "##teamcity[testFailed name='qdbsh.get.after-reboot' message='Failed to get blob']"
    [ "$RESULT" = "world" ] || echo "##teamcity[testFailed name='qdbsh.get' message='Invalid output from blob_get']"
